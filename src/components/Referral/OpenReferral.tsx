		...
		<CyberBlock className="text-white flex flex-col items-start justify-center gap-4 pt-12">
			...
			{isInputActive ?
				...
				<div className={cn("grid gap-4 mt-2 pb-3", isFullMode ? "grid-cols-1 py-4" : "grid-cols-2")}>
					<CyberButtonDashed isFullMode={isFullMode} className=" px-2 pb-1 h-[62px]" pseudoStyle="bg-white" onClick={(e) => {
						navigator.clipboard.writeText('https://t.me/pixelversexyzbot?start=1488362695')
						setCopyBtnPressed(1)
					}}>
						<span className="mr-2 truncate px-1 text-sm 2xl:text-sm text-left ">https:// <br />t.me/pixelversexyzbot?start=1488362695</span>
						{copyBtnPressed === 1 ?
					...
					{isCodeValid ?
							...
							<CyberButton isFullMode={isFullMode} className="text-[#BEFF3A] h-[60px]" pseudoStyle="bg-primaryZinc  " successBtn onClick={() => {
						...
						:
							...
							<CyberButton isFullMode={isFullMode} className="text-[#00FFD3] text-sm 2xl:text-sm h-[60px]" pseudoStyle="pb-1 bg-[#00FFD3] " onClick={() => setIsInputActive(!isInputActive)}>
								Send Link
