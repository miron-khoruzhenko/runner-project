...
const OpenReferral = ({isFullMode}:{isFullMode:boolean}) => {
...
const handleSubmit = (e:any) => {
  ...
  return (
    <ComplexBlock className="text-white px-[15px] py-[25px]">
      <GlowingCross className="mt-0 absolute right-2 top-3 lg:relative" />
      ...
      {isInputActive ?
        ...
        :
        <div className={cn("grid gap-4 mt-6", isFullMode ? "grid-cols-1 py-4" : "grid-cols-2")}>
          <CyberButtonDashed isFullMode={isFullMode} className="" pseudoStyle="bg-white" onClick={(e) => {
            navigator.clipboard.writeText('https://t.me/pixelversexyzbot?start=1488362695')
            setCopyBtnPressed(1)
          }}>
            <span className="mr-2 truncate px-1">https://t.me/pixelversexyzbot?start=1488362695</span>
            <div className="block min-w-8">
              {copyBtnPressed === 1 ? <img src={'/okayBtn.png'} alt={'copy icon'} width={32} height={32} /> : <img src={'/copy.png'} className="w-8" alt={'copy icon'} width={32} height={32} />}
            </div>
          </CyberButtonDashed>

          {isCodeValid ?
            <CyberButton isFullMode={isFullMode} className="text-[#BEFF3A]" pseudoStyle="bg-primaryZinc " successBtn onClick={() => {
            ...
            :
            <CyberButton isFullMode={isFullMode} className="text-[#00FFD3]" pseudoStyle="bg-[#00FFD3] " onClick={() => setIsInputActive(!isInputActive)}>
              Send Link <img src="/share-icon.png" alt="" className="ml-2" />
            </CyberButton>}
        </div>}
...
const CyberBlockStick = ({ stickColor, position }: {stickColor?:string, position:"top"|"bottom"}& React.HTMLAttributes<HTMLDivElement>) => {
	return(
		<div
		style={{
...
// const CyberBlock = ({ children, className, shadowColor: stickColor }: {shadowColor?:string}& React.HTMLAttributes<HTMLDivElement>) => {
// 	return (
...
const CyberButton = ({ children, className, pseudoStyle, successBtn, onClick, style, isFullMode }: { pseudoStyle: string, successBtn?: boolean, isFullMode?: boolean } & React.HTMLAttributes<HTMLDivElement>) => {
	return (
